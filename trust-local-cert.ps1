...
.DESCRIPTION
    ...
    WHAT THE IT ADMIN MUST DO FIRST (on the server):
      ...
      2. Copy rootCA.pem from that folder to a USB drive or network share
      3. Put rootCA.pem in the same folder as this script on each client device
         (or be ready to type the full path when this script asks)

    WHAT THIS SCRIPT DOES (on each client device):
      1. Checks for Administrator rights
      2. Locates rootCA.pem (or rootCA.cer) — next to this script, or asks for path
...
if (-not $isAdmin) {
    ...
    Write-Host "         Right-click the script, choose 'Run with PowerShell'," -ForegroundColor Yellow
    Write-Host "         then click Yes on the UAC prompt." -ForegroundColor Yellow
    Write-Host ""
...
if (-not $caCertPath) {
    ...
    Write-Host "         Copy 'rootCA.pem' from that folder to the same folder as" -ForegroundColor DarkGray
    Write-Host "         this script, then re-run trust-local-cert.ps1." -ForegroundColor DarkGray
    Write-Host ""
...
# This is the correct Windows-native way to add a CA to the machine trust store.
Write-Host ""
...
if ($certutilExit -eq 0) {
    Write-Host "         Server CA imported successfully." -ForegroundColor Green
    ...
    Write-Host "           - Try: certutil -addstore Root ""$caCertPath""" -ForegroundColor DarkGray
    Write-Host ""
    Read-Host "  Press Enter to exit"
    exit 1
}
